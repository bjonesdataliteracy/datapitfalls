...
export const rules: PitfallRule[] = [
  ...
  },
  {
    ...
    "detection_strategy": "Where values are labeled, compare the ratios of the rendered sizes to the ratios of the labels. Flag when the geometry cannot be reconciled with the numbers under one linear scale: two equal labels drawn at different sizes, a value twice another shown at near-equal size, or a mark whose size implies a value far from its label. Rule out innocent explanations first — a clearly stated non-linear (log) axis, or a non-zero baseline that still preserves the ordering — and only flag when no single consistent scale fits. In code, watch for bar lengths, heights, or widths set from a constant or from something other than the plotted value. When the marks are pictographs (icons, human silhouettes, unit-shaped figures) that scale in two dimensions, check for a truncated baseline *first*: a nonzero value axis is often the root cause and the 2-D area amplification only the visible symptom sitting on top of it. If the smallest value renders at roughly zero size (a figure that shrinks to nothing or is absent), treat truncated-y-axis as the primary finding and disproportionate encoding as the compounding one, rather than the reverse.\n",
  ...
  {
    "id": "incoherent-axis-scale",
    "name": "Incoherent Axis Scale",
    "domain": "Graphical Gaffes",
    "severity": "warning",
    "description": "An axis's tick labels are internally contradictory, so no consistent scale exists and no reader can map a position to a value. Adjacent ticks repeat the same label, the labels don't run monotonically (a smaller value sits where a larger one should, or the sequence reverses partway), or the same value appears on ticks that should be distinct. This is distinct from a truncated baseline (the scale is real, just not zero-based) and from an unevenly spaced axis (the labels are correct and ordered, only their spacing is disproportionate) — here the labels themselves are broken, so the axis lies about its own scale before any mark is even read. It extends the domain's encoding-integrity theme and is not a specific sub-pitfall in the book.\n",
    "detection_strategy": "Read the tick labels in order along the axis and check that they form one coherent scale: flag when adjacent ticks carry the same label, when the labels don't increase (or decrease) monotonically, when a smaller value is positioned above/beyond a larger one, or when a value is duplicated across ticks that should be distinct. Separate this from `unevenly-spaced-axis` (labels are valid and monotonic but the gaps aren't proportional) — reach for this rule only when the labels are self-contradictory, so no single scale can fit them. In code, watch for hand-set tick text overriding computed positions, a formatter that over-rounds distinct values to the same string (e.g. two ticks both rendering \"5'6\\\"\"), or categorical labels reused across positions.\n",
    "example_bad": "A height chart's right-hand axis reads, top to bottom, 5'0\", 5'9\", 5'7\", 5'6\", 5'6\", 5'6\", 5'6\", 5'5\", 5'5\", 5'5\": the same label repeats on adjacent ticks and 5'0\" sits above 5'9\", so the axis contradicts itself — a reader cannot decode any mark's height from it because the ruler itself is not a valid scale.\n",
    "example_good": "The ticks carry distinct, monotonically ordered labels at a consistent interval — 5'5\", 5'6\", 5'7\", 5'8\", 5'9\", 5'10\" — so every gridline maps unambiguously to one value. If rounding would collapse distinct values to the same label, precision is increased or the interval widened so each tick stays unique.\n",
    "remediation": "Let the plotting library derive ticks from the scale rather than hand-labeling them, and keep the tick formatter precise enough that adjacent labels stay distinct. Before publishing, read the axis top to bottom (or left to right) and confirm the labels are unique where they should be and run monotonically — an axis a reader can't decode is worse than no axis at all.\n",
    "references": [
      "Avoiding Data Pitfalls (Wiley, 2020), Pitfall 6: Graphical Gaffes (domain extension — not a specific sub-pitfall example in the book)",
      "Alberto Cairo, How Charts Lie (Norton, 2019) — dubious and self-contradictory chart scales",
      "https://www.avoidingdatapitfalls.com"
    ]
  },
  {
  ...
  },
  {
    ...
    "detection_strategy": "In code and charts, flag bar/column/area charts whose value axis starts above zero — explicit axis limits or scale domains excluding zero (matplotlib `set_ylim`, ggplot `coord_cartesian`/`ylim`, Vega-Lite `scale: {zero: false}`, an Excel minimum bound). Be suspicious of bar charts where a tiny percentage change fills most of the plot, and of a y-axis minimum set just below the smallest value. Scope the check to length-encoded marks, not line/dot charts where omitting zero can be fine — but note that pictorial or pictograph bars (icons, silhouettes, \"unit-shaped\" marks whose height carries the value) are length-encoded too and can be truncated just like plain rectangles. A strong tell is a missing or vanishing smallest mark: when the row with the lowest value renders at (or near) zero height — e.g. the shortest human silhouette is absent entirely — the baseline has been parked at that smallest value, which is textbook truncation. The pictograph costume hides it, because a zero-height figure reads as \"they forgot to draw it\" rather than as a cut-off axis.\n",
  ...
  {
    "id": "unevenly-spaced-axis",
    "name": "Unevenly Spaced Axis",
    "domain": "Graphical Gaffes",
    "severity": "warning",
    "description": "A quantitative axis gives each labeled tick an equal-width slot even though the numeric intervals between labels are unequal, so position along the axis no longer encodes the value proportionally. The most common case is time plotted on a categorical axis — a ten-year gap occupying the same distance as a one-year gap — but the same distortion happens on any numeric axis whose labels jump irregularly at even spacing (e.g. a height axis running 5'5\", 5'6\", 5'7\", 5'9\" across evenly spaced ticks). Because equal distance no longer means equal difference, slopes and gaps are misread: irregular sampling reads as steady progression, change over wide gaps looks abrupt, and change across narrow gaps looks gradual. This extends the domain's encoding-distortion theme but is not a specific sub-pitfall in the book.\n",
    "detection_strategy": "Read the axis tick labels as numbers or dates and check whether their numeric gaps match their visual spacing — equally spaced ticks whose labels jump irregularly (1969, 1972, 1973, 1977, ... 1985, 1995, 2000; or 5'5\", 5'6\", 5'7\", 5'9\") are the giveaway, especially on line charts where slope carries the message. The classic and most frequent form is a time axis: in code, flag temporal values plotted as strings or categories — years cast with `.astype(str)`, matplotlib or pandas plots over string-typed date columns, Vega-Lite `ordinal`/`nominal` (or Tableau discrete, Excel \"text axis\") encodings on a temporal field, and bar-style categorical axes used for a time series. But apply it to any quantitative axis: a numeric field placed on a categorical/ordinal scale, or hand-placed ticks at even intervals whose labels are not evenly spaced in value. Distinct from `misleading-interpolation` (even if connecting sparse points is acceptable, the axis itself must still give each interval its proportional width) and from `incoherent-axis-scale` (there the labels are broken or non-monotonic; here they are valid and ordered, only spaced disproportionately).\n",
    ...
    "example_good": "The same series is plotted on a continuous (quantitative or date-typed) axis, so the 1985-1995 gap is ten slots wide and the annual surveys cluster tightly; markers show where measurements actually occurred, and the varying steepness between them now honestly reflects change per unit of the axis variable.\n",
    "remediation": "Plot the axis variable on a continuous (quantitative or date-typed) scale so distance is proportional to difference — for time that means keeping dates as dates rather than strings, or switching the axis from categorical/discrete to continuous; for any other numeric field, use a linear scale rather than one evenly spaced tick per observation. Mark the observed points so sparse coverage stays visible, and if the gaps are large, consider annotating them rather than letting equal spacing paper over them.\n",
    "references": [
      "Avoiding Data Pitfalls (Wiley, 2020), Pitfall 6: Graphical Gaffes (domain extension — not a specific sub-pitfall example in the book)",
