...
datapitfalls takes some piece of data work — a chart, a code snippet, a plain-English description, or a whole document — and returns a structured report of the pitfalls it contains. The key insight is that good detection requires *grounding*: rather than asking an AI model to free-associate about what might be wrong, datapitfalls retrieves the relevant pitfall rules from a curated taxonomy and asks the model to reason over the input **in light of those rules** and the knowledge from *Avoiding Data Pitfalls*.
...
```
   ...
   │  PITFALL REPORT    │      names the pitfall, its domain and severity, why
...
- **Explainable.** A report that says "this is wrong" without saying *why* doesn't teach anything. Every finding explains itself.
- **Whole-chain.** We check the reasoning, not just the rendering. The most damaging pitfalls live upstream of the chart.
